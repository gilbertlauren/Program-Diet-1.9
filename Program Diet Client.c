...
#define FILE_MENU_NEXT 2
#define FILE_MENU_RESET 3
...
int	error;
int const port = 1178; 
char* const hostname = "10.10.46.16";
...
void insertbmrs(int number, float bmrs) {
...
void insertcalorie(float calorie) {
   // Allocate memory for new node;
	if((current->calorie)!=0){
		struct node *link = (struct node*) malloc(sizeof(struct node));
	}
	// Set value 
	current->calorie = calorie;
...
HMENU hMenu;
HWND hTinggi, hBerat, hBMR, hPW, hWP, hdrop, hExer, hNama, hUmur, hCal, hrdhm, hMakanCal, hLari, hBat, hnumber;
HWND hHasil[5]; hNamadata[5]; hBMRdata[5];
HWND chatbox, sendbox;
...
LRESULT CALLBACK WindowProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_COMMAND:
		...
		case SAVEFILE:
			...
			for(i=0;i<15;i++){
				SetWindowText(hNamadata[i], Databmr.Name[i]);
				sprintf(tempo,"%0.2f",Databmr.bmrs[i]);
				SetWindowText(hBMRdata[i], tempo);
				sprintf(tempo,"%0.2f",Databmr.calorie[i]);
				SetWindowText(hHasil[i], tempo);
			}
...
void AddMenus(HWND hWnd)
{
	...
	AppendMenu(hMenu, MF_POPUP, hContactMenu, "&Contact");
	AppendMenu(hContactMenu, MF_STRING, (HWND)CHAT, "&Start Chatting");
	AppendMenu(hContactMenu, MF_STRING, (HWND)STOP, "&Stop Chatting");
...
{	
	...
	{
	case 0:
		CreateWindowW(L"static", "1. Lunges\n    "
	...
	case 1:
		CreateWindowW(L"static", "2. Pushups\n    "
		...
		break;
	case 2:
		CreateWindowW(L"static", "3. Squats\n    "
	...
	case 3:
		CreateWindowW(L"static", "4. Standing overhead dumbbell presses\n    "
	...
	case 4:
		CreateWindowW(L"static", "5. Dumbbell rows\n	"
	...
	case 7:
		...
		"1. Lie on your right side with your left leg and foot stacked on top of your right leg and foot. "
		"Prop your upper body up by placing your right forearm on the ground, elbow directly under your shoulder.\n\n"
	...
	case 8:
		CreateWindowW(L"static", "9. Situps\n\n"
		...
		break;
	case 9:
		CreateWindowW(L"static", "10. Glute bridge\n\n"
...
void Savemenu(HWND hWnd)
{
	...
	CreateWindowW(L"static", L"No.", WS_VISIBLE | WS_CHILD | WS_BORDER | SS_CENTER , 50, 50, 50, 20, hWnd,NULL ,NULL,NULL);
	...
	for(i=0;i<15;i++){
		...
		hnumber = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_MULTILINE | SS_CENTER , 50, y, 50, 20, hWnd,NULL ,NULL,NULL);
		SetWindowText(hnumber, noo);
		hNamadata[i] = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_MULTILINE | SS_CENTER , 100, y, 100, 20, hWnd,NULL ,NULL,NULL);
		hBMRdata[i] = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_MULTILINE | SS_CENTER , 200, y, 100, 20, hWnd,NULL ,NULL,NULL);
		hHasil[i] = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_MULTILINE | SS_CENTER , 300, y, 100, 20, hWnd,NULL ,NULL,NULL);
		y+=20;
	}
...
void chatmenu(HWND hWnd){
	CreateWindowW(L"static", L"Chatroom", WS_VISIBLE | WS_CHILD | WS_BORDER | SS_CENTER , 10, 50, 100, 20, hWnd,NULL ,NULL,NULL);
	chatbox = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_VSCROLL| WS_BORDER | ES_MULTILINE | SS_LEFT , 10, 100, 465, 200, hWnd,NULL ,NULL,NULL);
	sendbox = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | WS_VSCROLL| WS_BORDER | ES_MULTILINE | SS_LEFT , 10, 350, 380, 50, hWnd,NULL ,NULL,NULL);

	CreateWindowW(L"button", L"Send", WS_VISIBLE | WS_CHILD | WS_BORDER | SS_LEFT, 400, 350, 75,50, hWnd, (HWND)SEND , NULL, NULL);
...
void online(){
	...
	error = connect(client, (struct sockaddr*)&serverInformation, sizeof(serverInformation));
